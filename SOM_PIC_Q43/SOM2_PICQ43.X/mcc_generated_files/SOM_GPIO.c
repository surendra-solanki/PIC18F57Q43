#include"SOM_GPIO.h"
#include <stdbool.h>

//////////////////////////RB4-SWITCH-INPUT ////////////////////
void SOM_SWITCH_SetHigh()
{
    SWITCH_SetHigh();
}

void SOM_SWITCH_SetLow()
{
    SWITCH_SetLow();
}

void SOM_SWITCH_Toggle()
{
    SWITCH_Toggle();   
}

bool SOM_SWITCH_GetValue()
{
    bool Return_Val = SWITCH_GetValue();
    return Return_Val;
}

void SOM_SWITCH_SetDigitalInput()
{
    SWITCH_SetDigitalInput();   
}

void SOM_SWITCH_SetDigitalOutput()
{
    SWITCH_SetDigitalOutput();  
}
 
//void SOM_SWITCH_SetHigh() //SetPullup()
//{
//    SWITCH_SWITCH_SetHigh()SetPullup();   
//}
 
void SOM_SWITCH_ResetPullup()
{
    SWITCH_ResetPullup();
}

void SOM_SWITCH_SetPushPull()
{
    SWITCH_SetPushPull();
}

void SOM_SWITCH_SetOpenDrain()
{
    SWITCH_SetOpenDrain();   
}

void SOM_SWITCH_SetAnalogMode()
{
    SWITCH_SetAnalogMode();   
}

void SOM_SWITCH_SetDigitalMode()
{
    SWITCH_SetDigitalMode();    
}
  


///////////////////////RB5-GPIO1-INPUT ////////////////////
void SOM_01_SetHigh()
{
  GPIO1_SetHigh();  
}
         
void SOM_01_SetLow()
{
    GPIO1_SetLow();
}

void SOM_01_Toggle()
{
    GPIO1_Toggle();  
}
     
bool SOM_01_GetValue()
{
    bool Return_Value = GPIO1_GetValue();
    return Return_Value;
}

void SOM_01_SetDigitalInput()
{
    GPIO1_SetDigitalInput(); 
}
       
void SOM_01_SetDigitalOutput()
{
    GPIO1_SetDigitalOutput();   
}

void SOM_01_SetPullup()
{
    GPIO1_SetPullup();   
}

void SOM_01_ResetPullup()
{
    GPIO1_ResetPullup();   
}
 
void SOM_01_SetPushPull()
{
    GPIO1_SetPushPull();    
}

void SOM_01_SetOpenDrain()
{
    GPIO1_SetOpenDrain();  
}

void SOM_01_SetAnalogMode()
{
    GPIO1_SetAnalogMode();    
}

void SOM_01_SetDigitalMode()
{
    GPIO1_SetDigitalMode();
}




///////////////////////RC2-GPIO2-INPUT ////////////////////
void SOM_02_SetHigh()
{
  GPIO2_SetHigh();  
}
         
void SOM_02_SetLow()
{
    GPIO2_SetLow();
}

void SOM_02_Toggle()
{
    GPIO2_Toggle();  
}
     
bool SOM_02_GetValue()
{
    bool Return_Value = GPIO2_GetValue();
    return Return_Value;
}
    
void SOM_02_SetDigitalInput()
{
    GPIO2_SetDigitalInput(); 
}
       
void SOM_02_SetDigitalOutput()
{
    GPIO2_SetDigitalOutput();   
}

void SOM_02_SetPullup()
{
    GPIO2_SetPullup();   
}

void SOM_02_ResetPullup()
{
    GPIO2_ResetPullup();   
}
 
void SOM_02_SetPushPull()
{
    GPIO2_SetPushPull();    
}

void SOM_02_SetOpenDrain()
{
    GPIO2_SetOpenDrain();  
}

void SOM_02_SetAnalogMode()
{
    GPIO2_SetAnalogMode();    
}

void SOM_02_SetDigitalMode()
{
    GPIO2_SetDigitalMode();
}


///////////////////////RC3->GPIO3->INPUT ////////////////////

void SOM_03_SetHigh()
{
  GPIO3_SetHigh();  
}
         
void SOM_03_SetLow()
{
    GPIO3_SetLow();
}

void SOM_03_Toggle()
{
    GPIO3_Toggle();  
}
     
bool SOM_03_GetValue()
{
    bool Return_Value = GPIO3_GetValue();
    return Return_Value;
}
    
void SOM_03_SetDigitalInput()
{
    GPIO3_SetDigitalInput(); 
}
       
void SOM_03_SetDigitalOutput()
{
    GPIO3_SetDigitalOutput();   
}

void SOM_03_SetPullup()
{
    GPIO3_SetPullup();   
}

void SOM_03_ResetPullup()
{
    GPIO3_ResetPullup();   
}
 
void SOM_03_SetPushPull()
{
    GPIO3_SetPushPull();    
}

void SOM_03_SetOpenDrain()
{
    GPIO3_SetOpenDrain();  
}

void SOM_03_SetAnalogMode()
{
    GPIO3_SetAnalogMode();    
}

void SOM_03_SetDigitalMode()
{
    GPIO3_SetDigitalMode();
}



///////////////////////RD2->GPIO4->INPUT ////////////////////

void SOM_04_SetHigh()
{
  GPIO4_SetHigh();  
}
         
void SOM_04_SetLow()
{
    GPIO4_SetLow();
}

void SOM_04_Toggle()
{
    GPIO4_Toggle();  
}
     
bool SOM_04_GetValue()
{
    bool Return_Value = GPIO4_GetValue();
    return Return_Value;
}
    
void SOM_04_SetDigitalInput()
{
    GPIO4_SetDigitalInput(); 
}
       
void SOM_04_SetDigitalOutput()
{
    GPIO4_SetDigitalOutput();   
}

void SOM_04_SetPullup()
{
    GPIO4_SetPullup();   
}

void SOM_04_ResetPullup()
{
    GPIO4_ResetPullup();   
}
 
void SOM_04_SetPushPull()
{
    GPIO4_SetPushPull();    
}

void SOM_04_SetOpenDrain()
{
    GPIO4_SetOpenDrain();  
}

void SOM_04_SetAnalogMode()
{
    GPIO4_SetAnalogMode();    
}

void SOM_04_SetDigitalMode()
{
    GPIO4_SetDigitalMode();
}



///////////////////////RD3->GPIO5->INPUT ////////////////////

void SOM_05_SetHigh()
{
  GPIO5_SetHigh();  
}
         
void SOM_05_SetLow()
{
    GPIO5_SetLow();
}

void SOM_05_Toggle()
{
    GPIO5_Toggle();  
}
     
bool SOM_05_GetValue()
{
    bool Return_Value = GPIO5_GetValue();
    return Return_Value;
}
    
void SOM_05_SetDigitalInput()
{
    GPIO5_SetDigitalInput(); 
}
       
void SOM_05_SetDigitalOutput()
{
    GPIO5_SetDigitalOutput();   
}

void SOM_05_SetPullup()
{
    GPIO5_SetPullup();   
}

void SOM_05_ResetPullup()
{
    GPIO5_ResetPullup();   
}
 
void SOM_05_SetPushPull()
{
    GPIO5_SetPushPull();    
}

void SOM_05_SetOpenDrain()
{
    GPIO5_SetOpenDrain();  
}

void SOM_05_SetAnalogMode()
{
    GPIO5_SetAnalogMode();    
}

void SOM_05_SetDigitalMode()
{
    GPIO5_SetDigitalMode();
}




///////////////////////RE1->GPIO7->OUTPUT ////////////////////

void SOM_07_SetHigh()
{
  GPIO7_SetHigh();  
}
         
void SOM_07_SetLow()
{
    GPIO7_SetLow();
}

void SOM_07_Toggle()
{
    GPIO7_Toggle();  
}
     
bool SOM_07_GetValue()
{
    bool Return_Value = GPIO7_GetValue();
    return Return_Value;
}
    
void SOM_07_SetDigitalInput()
{
    GPIO7_SetDigitalInput(); 
}
       
void SOM_07_SetDigitalOutput()
{
    GPIO7_SetDigitalOutput();   
}

void SOM_07_SetPullup()
{
    GPIO7_SetPullup();   
}

void SOM_07_ResetPullup()
{
    GPIO7_ResetPullup();   
}
 
void SOM_07_SetPushPull()
{
    GPIO7_SetPushPull();    
}

void SOM_07_SetOpenDrain()
{
    GPIO7_SetOpenDrain();  
}

void SOM_07_SetAnalogMode()
{
    GPIO7_SetAnalogMode();    
}

void SOM_07_SetDigitalMode()
{
    GPIO7_SetDigitalMode();
}




///////////////////////RE2->GPIO8->OUTPUT ////////////////////

void SOM_08_SetHigh()
{
  GPIO8_SetHigh();  
}
         
void SOM_08_SetLow()
{
    GPIO8_SetLow();
}

void SOM_08_Toggle()
{
    GPIO8_Toggle();  
}
     
bool SOM_08_GetValue()
{
    bool Return_Value = GPIO8_GetValue();
    return Return_Value;
}
    
void SOM_08_SetDigitalInput()
{
    GPIO8_SetDigitalInput(); 
}
       
void SOM_08_SetDigitalOutput()
{
    GPIO8_SetDigitalOutput();   
}

void SOM_08_SetPullup()
{
    GPIO8_SetPullup();   
}

void SOM_08_ResetPullup()
{
    GPIO8_ResetPullup();   
}
 
void SOM_08_SetPushPull()
{
    GPIO8_SetPushPull();    
}

void SOM_08_SetOpenDrain()
{
    GPIO8_SetOpenDrain();  
}

void SOM_08_SetAnalogMode()
{
    GPIO8_SetAnalogMode();    
}

void SOM_08_SetDigitalMode()
{
    GPIO8_SetDigitalMode();
}



///////////////////////RF2->GPIO9->OUTPUT ////////////////////

void SOM_09_SetHigh()
{
  GPIO9_SetHigh();  
}
         
void SOM_09_SetLow()
{
    GPIO9_SetLow();
}

void SOM_09_Toggle()
{
    GPIO9_Toggle();  
}
     
bool SOM_09_GetValue()
{
    bool Return_Value = GPIO9_GetValue();
    return Return_Value;
}
    
void SOM_09_SetDigitalInput()
{
    GPIO9_SetDigitalInput(); 
}
       
void SOM_09_SetDigitalOutput()
{
    GPIO9_SetDigitalOutput();   
}

void SOM_09_SetPullup()
{
    GPIO9_SetPullup();   
}

void SOM_09_ResetPullup()
{
    GPIO9_ResetPullup();   
}
 
void SOM_09_SetPushPull()
{
    GPIO9_SetPushPull();    
}

void SOM_09_SetOpenDrain()
{
    GPIO9_SetOpenDrain();  
}

void SOM_09_SetAnalogMode()
{
    GPIO9_SetAnalogMode();    
}

void SOM_09_SetDigitalMode()
{
    GPIO9_SetDigitalMode();
}



///////////////////////RF3->LED->OUTPUT ////////////////////

void SOM_LED_SetHigh()
{
    LED_SetHigh();  
}
         
void SOM_LED_SetLow()
{
    LED_SetLow();
}

void SOM_LED_Toggle()
{
    LED_Toggle();  
}
     
bool SOM_LED_GetValue()
{
    bool Return_Value = LED_GetValue();
    return Return_Value;
}
    
void SOM_LED_SetDigitalInput()
{
    LED_SetDigitalInput(); 
}
       
void SOM_LED_SetDigitalOutput()
{
    LED_SetDigitalOutput();   
}

void SOM_LED_SetPullup()
{
    LED_SetPullup();   
}

void SOM_LED_ResetPullup()
{
    LED_ResetPullup();   
}
 
void SOM_LED_SetPushPull()
{
    LED_SetPushPull();    
}

void SOM_LED_SetOpenDrain()
{
    LED_SetOpenDrain();  
}

void SOM_LED_SetAnalogMode()
{
    LED_SetAnalogMode();    
}

void SOM_LED_SetDigitalMode()
{
    LED_SetDigitalMode();
}



///////////////////////RF4->GPIO10->OUTPUT ////////////////////

void SOM_10_SetHigh()
{
  GPIO10_SetHigh();  
}
         
void SOM_10_SetLow()
{
    GPIO10_SetLow();
}

void SOM_10_Toggle()
{
    GPIO10_Toggle();  
}
     
bool SOM_10_GetValue()
{
    bool Return_Value = GPIO10_GetValue();
    return Return_Value;
}
    
void SOM_10_SetDigitalInput()
{
    GPIO10_SetDigitalInput(); 
}
       
void SOM_10_SetDigitalOutput()
{
    GPIO10_SetDigitalOutput();   
}

void SOM_10_SetPullup()
{
    GPIO10_SetPullup();   
}

void SOM_10_ResetPullup()
{
    GPIO10_ResetPullup();   
}
 
void SOM_10_SetPushPull()
{
    GPIO10_SetPushPull();    
}

void SOM_10_SetOpenDrain()
{
    GPIO10_SetOpenDrain();  
}

void SOM_10_SetAnalogMode()
{
    GPIO10_SetAnalogMode();    
}

void SOM_10_SetDigitalMode()
{
    GPIO10_SetDigitalMode();
}




///////////////////////RF6->GPIO12->OUTPUT ////////////////////

void SOM_12_SetHigh()
{
  GPIO12_SetHigh();  
}
         
void SOM_12_SetLow()
{
    GPIO12_SetLow();
}

void SOM_12_Toggle()
{
    GPIO12_Toggle();  
}
     
bool SOM_12_GetValue()
{
    bool Return_Value = GPIO12_GetValue();
    return Return_Value;
}
    
void SOM_12_SetDigitalInput()
{
    GPIO12_SetDigitalInput(); 
}
       
void SOM_12_SetDigitalOutput()
{
    GPIO12_SetDigitalOutput();   
}

void SOM_12_SetPullup()
{
    GPIO12_SetPullup();   
}

void SOM_12_ResetPullup()
{
    GPIO12_ResetPullup();   
}
 
void SOM_12_SetPushPull()
{
    GPIO12_SetPushPull();    
}

void SOM_12_SetOpenDrain()
{
    GPIO12_SetOpenDrain();  
}

void SOM_12_SetAnalogMode()
{
    GPIO12_SetAnalogMode();    
}

void SOM_12_SetDigitalMode()
{
    GPIO12_SetDigitalMode();
}




///////////////////////RF7->GPIO13->OUTPUT ////////////////////

void SOM_13_SetHigh()
{
  GPIO13_SetHigh();  
}
         
void SOM_13_SetLow()
{
    GPIO13_SetLow();
}

void SOM_13_Toggle()
{
    GPIO13_Toggle();  
}
     
bool SOM_13_GetValue()
{
    bool Return_Value = GPIO13_GetValue();
    return Return_Value;
}
    
void SOM_13_SetDigitalInput()
{
    GPIO13_SetDigitalInput(); 
}
       
void SOM_13_SetDigitalOutput()
{
    GPIO13_SetDigitalOutput();   
}

void SOM_13_SetPullup()
{
    GPIO13_SetPullup();   
}

void SOM_13_ResetPullup()
{
    GPIO13_ResetPullup();   
}
 
void SOM_13_SetPushPull()
{
    GPIO13_SetPushPull();    
}

void SOM_13_SetOpenDrain()
{
    GPIO13_SetOpenDrain();  
}

void SOM_13_SetAnalogMode()
{
    GPIO13_SetAnalogMode();    
}

void SOM_13_SetDigitalMode()
{
    GPIO13_SetDigitalMode();
}

